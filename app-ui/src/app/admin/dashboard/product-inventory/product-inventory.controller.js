...
function ProductInventoryController($state, ProductInventoryService, StockCardsService, $rootScope, ProductStockCardService,_) {
  var ctrl = this;
  ...
  function loadProductInventory(){
	  ...
	  ProductInventoryService.listProductInventoryView(ctrl.company.id).then(function(response){
  ...
  ctrl.openStockCard = function(productInventory){
	  ProductStockCardService.listByCompanyAndProduct(ctrl.company.id, productInventory.product.id).then(function(response){
