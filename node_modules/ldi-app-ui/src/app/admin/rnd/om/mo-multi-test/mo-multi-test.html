...
<div class="admin-header">
    <div class="title horizontal-align -between">
        <h4 class="bg">Manufacturing Order - Multi test</h4>
    </div>
    <hr class="devider">
</div>
...
<form novalidate ng-submit="$ctrl.submitForm();" method="post">
    <div class="form-content">
        ...
        <div class="form-content">
            ...
            <button type="button" class="btn btn-create btn-md" ng-click="$ctrl.testRun()">Add to Test Run</button>
            <button type="button" class="btn btn-warning btn-md" ng-click="$ctrl.reset()">Reset</button>
        </div>
            <!-- <pre>{{$ctrl.tests | json}}</pre> -->
        ...
        <table ng-if="$ctrl.mo.inventoryList.length > 0" class="table table-list">
            ...
            <tbody>
                <tr style="{{(inv.result > 0) ? 'background-color: yellow' : 'background-color: white'}}" ng-repeat="inv in $ctrl.mo.inventoryList">
                    ...
                    <td>
                        {{inv.result = $ctrl.processLacking(inv.quantity, inv.moQuantity)}}
                    </td>
                </tr>
            </tbody>
        </table>
