...
<div class="form-content">
    ...
    <table ng-if="$ctrl.costing.moInventory.inventoryList.length > 0" class="table table-list">
        ...
        <tbody>
            <tr ng-repeat="moCostingInv in $ctrl.costing.moCostingInventories">
                <td>
                    {{moCostingInv.inventory.item.name}} {{inv.item.code}}
                </td>
                <td>
                    {{moCostingInv.inventory.item.type.code}}
                </td>
                <td>
                    {{moCostingInv.inventory.controlNumber}}
                </td>

                <td>
                    {{moCostingInv.inventory.number = $ctrl.findIngredientQuantity(moCostingInv.inventory.item.code) * $ctrl.costing.moInventory.batchSize}}
                </td>
                <td>
                    {{moCostingInv.cost}}
                </td>
