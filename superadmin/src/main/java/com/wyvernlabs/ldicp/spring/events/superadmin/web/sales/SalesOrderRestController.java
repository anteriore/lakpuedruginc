...
public class SalesOrderRestController {
	...
	public SalesOrder approveSalesOrder(@PathVariable Long prfId) {
		SalesOrder salesOrder = salesOrderRepository.getOne(prfId);
	...
	public List<CancelSalesOrderProduct> getCanceleldReqsOfSO(@PathVariable Long soId) {
		SalesOrder so = salesOrderRepository.getOne(soId);
	...
	public List<SalesOrder> listNotCompletedByCompanyAndDepot(@PathVariable Long companyId, @PathVariable Long depotId,
			@PathVariable OrderSlipType type) {
		Company company = companyRepository.getOne(companyId);
		Depot depot = depotRepository.getOne(depotId);
	...
	public Map<String, Object> getTotalAmountAndQuantity(@PathVariable Long depotId, @PathVariable Date startDate,
			@PathVariable Date endDate, @PathVariable Long salesRepId) {
		SalesRep salesRep = salesRepRepository.getOne(salesRepId);
		Depot depot = depotRepository.getOne(depotId);
		String[] status = { "Completed", "In Transit", "Incomplete" };
		List<SalesOrder> salesOrders = salesOrderRepository.findByDepotAndStatusInAndDateBetween(depot, status,
				startDate, endDate);
		Double totalAmount = salesOrders.stream().filter(so -> so.getClient().getSalesRep() == salesRep)
				.mapToDouble(so -> so.getTotalAmount()).sum();
		int totalQuantity = salesOrders.stream().filter(so -> so.getClient().getSalesRep() == salesRep)
				.mapToInt(so -> so.getTotalQuantity()).sum();
		Map map = new HashMap();
