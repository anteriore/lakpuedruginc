...
public class ProductMovementService {
	...
	public ProductMovement saveProductMovement(ProductMovement productMovement) {
		...
		productStockCard.setDate(new Date());
		productStockCard.setQuantity(productMovement.getQuantity());
		productStockCard.setUser(productMovement.getRequestedBy());
		productStockCard.setAction("PM-" + productMovement.getType());
		Product product = productRepository.findByFinishedGoodAndLotNumber(
				productMovement.getProduct().getFinishedGood(), 
				productMovement.getProduct().getLotNumber());
		Depot depot = depotRepository.findOne(productMovement.getDepot().getId());
		if(productMovement.getType().equals(ProductMovementType.IN)) {
			if(product != null) {
				ProductInventory productInventory = productInventoryRepository.findByProductAndDepot(product, depot);
					if(productInventory != null) {
						productInventory.setQuantity(productInventory.getQuantity() + productMovement.getQuantity());
						productInventoryRepository.save(productInventory);
					}else {
						productInventory = new ProductInventory();
						productInventory.setDepot(depot);
						productInventory.setProduct(product);
						productInventory.setQuantity(productMovement.getQuantity());
						productInventoryRepository.save(productInventory);
					}
			}else {
				product = productRepository.save(productMovement.getProduct());
			...
			productStockCard.setSign("IN");
			
		}else if(productMovement.getType().equals(ProductMovementType.OUT) || productMovement.getType().equals(ProductMovementType.SHIP)) {
			ProductInventory productInventory = productInventoryRepository.findByProductAndDepot(product, depot);
			productInventory.setQuantity(productInventory.getQuantity() - productMovement.getQuantity());
			productInventoryRepository.save(productInventory);
			
			productStockCard.setProduct(product);
			productStockCard.setSign("OUT");
		}
	...
	public ProductMovement saveProductMovement(ProductMovement productMovement) {
		...
		for(ProductMovementItem productMovementItem : productMovement.getProducts()) {
			ProductInventory inventory = productInventoryRepository.findByProductAndDepot(productMovementItem.getProduct(), productMovement.getDepot());
			...
			productStockCardService.saveProductStockCard("FGRIS", productMovement.getCompany(), productMovement.getDepot(), productMovement.getDate(), productMovementItem.getQuantity(), productMovement.getRemarks(), productMovement.getRequestedBy(), productMovement.getType().toString(), productMovementItem.getProduct());
