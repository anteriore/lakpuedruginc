...
public class InventoryMovementService {
	...
	public InventoryMovement saveInventoryMovement(InventoryMovement inventoryMovement) {
		...
		for (InventoryMovementItem item : inventoryMovement.getInventory()) {
			Inventory inventory = inventoryRepository.findByControlNumberAndCompany(item.getControlNumber(),
					inventoryMovement.getCompany());
			...
			stockCardService.saveStockCard("MRIS(" + inventoryMovement.getClassification() + ")",
					companyRepository.getOne(inventoryMovement.getCompany().getId()), item.getControlNumber(),
					new Date(), item.getQuantity(), inventoryMovement.getRemarks(),
					inventoryMovement.getType().toString(), inventoryMovement.getRequestedBy());
		}
