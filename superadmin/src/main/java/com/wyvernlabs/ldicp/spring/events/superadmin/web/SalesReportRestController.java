...
public class SalesReportRestController {
	...
	public Map<String, Object> itemSalesReportAmount(@PathVariable Long depotId, @PathVariable Date startDate, @PathVariable Date endDate, @PathVariable Long itemId){
		...
		for(SalesSlip salesSlip : salesSlips) {
			quantity += salesSlip.getOrderedProducts().stream().filter(op -> op.getProduct() == product).mapToInt(op -> op.getQuantity()).sum();
			amount += salesSlip.getOrderedProducts().stream().filter(op -> op.getProduct() == product).mapToDouble(op -> op.getAmount()).sum();
		}
	...
	/*
	@RequestMapping("/item-sales-report/depot/{depotId}/start/{startDate}/end/{endDate}/item/{itemId}")
	public List<Map<String, Object>> itemSalesReportAmountReport(@PathVariable Long depotId, @PathVariable Date startDate, @PathVariable Date endDate, @PathVariable Long itemId){
		Depot depot = depotRepository.findOne(depotId);
		Product product = productRepository.findOne(itemId);
...
/*
		for(int i = startDate.getMonth(); i < endDate.getMonth() ; i++) {
			List<SalesSlip> salesSlips = salesSlipRepository.findByDepotAndMonthDate(depot, i);
			int quantity = 0;
			Double amount = 0D;
			Map map = new HashMap();
			for(SalesSlip salesSlip : salesSlips) {
			...
			map.put("item", product.getFinishedGood().getName());
	...
	@RequestMapping("/item-sales-report/depot/{depotId}/start/{startDate}/end/{endDate}/category/{categoryId}/division/{divisionId}")
	public Map<String, Object> itemSalesReportAmount(@PathVariable Long depotId, @PathVariable Date startDate, @PathVariable Date endDate, @PathVariable Long categoryId, @PathVariable Long divisionId){
		Depot depot = depotRepository.findOne(depotId);
		ProductCategory category = categoryRepository.findOne(categoryId);
		ProductDivision division = divisionRepository.findOne(divisionId);
		List<SalesSlip> salesSlips = salesSlipRepository.findByDepotAndDateBetween(depot, startDate, endDate);
		int quantity = 0;
		Double amount = 0D;
		Map map = new HashMap();
		for(SalesSlip salesSlip : salesSlips) {
			quantity += salesSlip.getOrderedProducts().stream().filter(op -> op.getProduct().getCategory() == category && op.getProduct().getDivision() == division).mapToInt(op -> op.getQuantity()).sum();
			amount += salesSlip.getOrderedProducts().stream().filter(op -> op.getProduct().getCategory() == category && op.getProduct().getDivision() == division).mapToDouble(op -> op.getAmount()).sum();
		}
