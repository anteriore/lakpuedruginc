...
const InputForm = (props) => {
  ...
  const [formType, setFormType] = useState(
    _.find(formDetails.form_items, { name: 'adjustment' }).initialValue
  );
  ...
  const renderNewAdjForm = () =>
    tempNewAdjForm.form_items.map((item) => (
      <FormItem onFail={onFail} key={item.name} item={item} />
    ));

  const renderWithAdjForm = () =>
    tempWithAdjForm.form_items.map((item) => (
      <FormItem onFail={onFail} key={item.name} item={item} />
    ));
