...
const InputForm = (props) => {
  ...
  const [formType, setFormType] = useState(
    _.find(formDetails.form_items, { name: 'manual' }).initialValue
  );
  const [form] = useForm();
  ...
  const handleChangeRR = useCallback(
    (value) => {
  ...
  const handleAddAccount = async () => {
    const accountValidation = ['accountTitles', 'department', 'group', 'area', 'debit', 'credit'];
    try {
      const values = await form.validateFields(accountValidation);
      const newAddedAccounts = _.clone(addedAccounts);
  ...
  const renderAutoForm = () =>
    tempAutoForm.form_items.map((item) => <FormItem onFail={onFail} key={item.name} item={item} />);

  const renderManualForm = () =>
    tempManualForm.form_items.map((item) => (
    ...
    <Descriptions column={7} title="Accounts" layout="vertical" size="small" bordered>
      {_.dropRight(_.drop(tempMainForm.form_items, 3), 3).map((item) => (
        <Descriptions.Item label={item.label}>
          <FormItem disableLabel noStyle onFail={onFail} key={item.name} item={item} />
        </Descriptions.Item>
      ))}
      ...
      <Row>
        <Col span={20} offset={1}>
          {contentLoading ? (
            ...
            <Form scrollToFirstError form={form} {...styles.formLayout}>
              {_.dropRight(tempMainForm.form_items, 7).map((item) => (
                <FormItem onFail={onFail} key={item.name} item={item} />
              ))}
              {subContentLoading ? <Skeleton /> : formType ? renderManualForm() : renderAutoForm()}
              {renderAccountsTable()}
              <br />
              <Table columns={modAccountColumn} dataSource={addedAccounts} pagination={false} />
              <br />
