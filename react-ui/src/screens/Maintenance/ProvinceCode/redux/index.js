...
export const listProvinceCode = createAsyncThunk('list', async(payload, thunkAPI) => {
...
export const createProvinceCode = createAsyncThunk('createUnit', async(payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
...
export const updateProvinceCode = createAsyncThunk('updateUnit', async(payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
  const response = await axiosInstance.post(`/rest/province-codes?token=${accessToken}`, payload);
...
export const deleteProvinceCode = createAsyncThunk('deleteUnit', async(payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
  const { id } = payload;
  const response = await axiosInstance.post(`/rest/province-codes/delete?token=${accessToken}`, id);
