...
export const createBankAccount = createAsyncThunk('createBankAccount', async (payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
...
export const updateBankAccount = createAsyncThunk('updateBankAccount', async (payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
  const response = await axiosInstance.post(`/rest/bank-accounts?token=${accessToken}`, payload);
...
export const deleteBankAccount = createAsyncThunk('deleteBankAccount', async (payload, thunkAPI) => {
  const accessToken = thunkAPI.getState().auth.token;
  const { id } = payload;
  const response = await axiosInstance.post(`/rest/bank-accounts/delete?token=${accessToken}`, id);
